...
本工具设计用于**长时间无人值守**运行,同时保证**有人需要使用电脑时绝不与之抢设备**:

- **出错不中断**:单条查询失败默认跳过并继续,只有连续失败达到 `max_consecutive_failures`(默认 10 次)才判定为页面结构变化 / 登录失效而停止。这避免了一次网络抖动导致整批中断。
- **环境级故障自动等待恢复**:若 Edge 中途被关闭,工具进入「等待恢复」状态,每 `recovery_poll_s` 秒检查一次;你重新打开 Edge 并登录后,它自动从断点重试当前条目,不前进、不丢失进度。
- **有人接手立即暂停**:运行时若检测到鼠标被移动、键盘被按下、或前台窗口被切走,工具立即让出控制权并暂停,**不打断正在进行的保存**(避免写出半截文件),下一个操作前停止。人离开 `resume_idle_s`(默认 5 秒)后自动恢复,当前任务完整重做。
- **紧急制动**:把鼠标快速甩到屏幕任意角落会触发 PyAutoGUI 的 FailSafe,工具将其转为「暂停等待」而非崩溃。
- **迷你控制条**:运行时屏幕左上角的迷你条新增「暂停 / 继续」按钮,可手动接管;状态文字会实时显示「查询运行中…」或「⏸ 已暂停(等待人工)」。
- **断点续跑**:任何原因结束(停止 / 连续失败 / 你关机)后,再次「开始查询」会询问是否从上次进度继续。

> 暂停期间工具不与任何窗口交互,电脑完全由你操控。恢复后会重新激活 Edge 窗口并同步基准,不会因为你的修复操作被误判为新的介入。
...
| `max_consecutive_failures` | `10` | 连续失败达此次数才停止(熔断阈值);设为 `1` 等价于「任何失败都停」 |
| `stop_on_error` | `false` | `true`=任一失败立即停止(兼容旧配置,不推荐无人值守) / `false`=走熔断阈值 |
| `recovery_poll_s` | `10` | 致命错误后(如 Edge 关闭)检查环境恢复的周期(秒) |
| `pause_on_intervention` | `true` | 是否启用用户介入守卫(有人接手即暂停) |
| `intervention_check` | `auto` | 检测模式:`auto`(三信号融合) / `mouse`(仅鼠标) / `frontmost`(仅前台) / `off`(关闭) |
| `resume_idle_s` | `5.0` | 人离开多少秒后自动恢复(秒) |
| `intervention_wait_timeout_s` | `0` | 暂停最长等待(秒);`0`=无限等待人工 |
